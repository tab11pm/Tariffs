...
export const TariffCard = ({ card, onClick, isActive }: TariffCardProps) => {
	...
	return (
		<Card
			key={card.id}
			className={` card-background relative overflow-hidden border-2 ${isActive ? 'border-(--primary)' : 'border-[#484D4E]'} px-8 py-10 hover:-translate-y-2 hover:translate-x-1 cursor-pointer  transition-transform ease-in duration-150 hover:shadow-lg  relative shadow-orange-300`}
		>
			<button
				className="block  md:overflow-hidden w-full h-full md:space-y-6"
				onClick={() => onClick(card.id)}
			>
				<div className="flex h-1 md:h-auto justify-end md:justify-between gap-2">
					...
					<div></div>
					<div className="relative md:static -top-2 rounded-lg md:text-3xl text-(--primary) ">
						{card.is_best && 'хит!'}
					</div>
				</div>
